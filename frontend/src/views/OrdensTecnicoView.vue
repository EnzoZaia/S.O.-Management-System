...
const formConclusao = ref({ tipo: '', patrimonio: '',  descricao: '' })
...
async function pausarOS() {
  ...
  if (statusEscolhido) {
    let tituloDetalhe = 'Detalhes da Pausa'
    let placeholderDetalhe = 'Descreva o motivo...'

    if (statusEscolhido === 'AGUARDANDO_MATERIAL') {
      tituloDetalhe = 'Quais materiais faltam?'
    ...
    const { value: justificativa } = await Swal.fire({ 
      title: tituloDetalhe, 
      input: 'text', 
      inputPlaceholder: placeholderDetalhe,
      showCancelButton: true, 
...
async function enviarConclusao() {
  if (!formConclusao.value.tipo) return
  try {
