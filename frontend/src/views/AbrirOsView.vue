                  ...
                  <div class="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex justify-between items-start mb-2">
                      ...
                      <span class="text-[10px] font-bold text-gray-500 bg-gray-50 border border-gray-100 px-2 py-1 rounded-md">
                        {{ formatarData(evento.data_conclusao || evento.dt_alteracao || evento.data_registro) }}
                      </span>
                    </div>
                    
                    <p class="text-sm text-gray-700 mt-2 italic">"{{ evento.descricao || evento.observacao || evento.justificativa || evento.desc_historico || 'Serviço concluído sem detalhes adicionais.' }}"</p>
                    
                    <div class="mt-3 pt-3 border-t border-gray-100 flex items-center gap-2">
                      <span class="bg-gray-100 p-1 rounded text-gray-500 text-xs">👤</span>
                      <span class="text-xs font-semibold text-gray-600">Responsável: {{ evento.tecnico_nome || evento.usuario_nome || 'N/I' }}</span>
                    </div>
                  </div>
                </div>
              </div>
