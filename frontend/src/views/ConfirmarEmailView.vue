  ...
  <div class="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-4">
    <div class="max-w-md w-full bg-white rounded-3xl shadow-xl border border-gray-100 p-8 text-center animate-fade-in">
      ...
      <div v-else-if="status === 'success'" class="flex flex-col items-center">
        ...
        <p class="text-sm text-gray-500 mt-3 mb-8 leading-relaxed">Sua conta foi ativada com sucesso. Você já pode acessar o sistema.</p>
      ...
      <div v-else class="flex flex-col items-center">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Link Inválido</h2>
        <p class="text-sm text-gray-500 mt-3 mb-8 leading-relaxed">O link de confirmação não é válido ou já foi utilizado.</p>
...
function irParaLogin() { router.push('/') }
</script>

<style scoped> .animate-fade-in { animation: fadeIn 0.4s ease-out; } @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } } </style>